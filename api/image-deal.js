...
const pathToBase64=(path, parseQuality)=> {
  return new Promise(function(resolve, reject) {
    ...
    if (typeof plus === 'object') {
      ...
      let index = path.lastIndexOf("."); //获取图片地址最后一个点的位置  
      let img_type ='jpg'; //path.substring(index + 1, path.length); //截取图片类型如png jpg
      let img_ori_path = path.substring(0, index); //截取图片原始路径
