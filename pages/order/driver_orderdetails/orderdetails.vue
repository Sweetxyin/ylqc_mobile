	...
	<view class="order_map">
		...
		<view class="order_info">
			...
			<view class="address_info" v-for="(item,index) in orderList" :key="index">
				...
				<!-- 途径地址信息 -->
				<!-- <view class="orad_list" v-show="orderList.length != 0" v-for="(oradItem,oradIndex) in roadList" :key="oradIndex">
					<view class="orad_item">
						<view class="address_list">
							<u-icon name="car-fill" color="#666666" size="16"></u-icon>
							<text style="font-weight: bold; font-size: 30rpx;">{{oradItem.receAddress}}</text>
						</view>
					...
					<view class="orad_item">
						<view class="address_txt">
							<text>{{oradItem.receFullAddress}}</text>
						</view>
						<view class="order_state" v-show="oradItem.state==='订单已完成'">
							<text style="color: darkgray;">{{oradItem.state}}</text>
						</view>
						<view class="order_state" v-show="oradItem.state==='订单进行中'">
					...
					<view class="orad_item">
						...
						<view class="service_button" @click="toService(item)">
							<u-button type="info" shape="circle" :plain="true" text="送达确认" size="mini"></u-button>
						</view>
					</view>
				...
				<view class="other_info">
					...
					<view class="other_item">
						<text>订单时间</text>
						<text style="padding-right: 15rpx;">{{item.createTime}}</text>
					</view>
					<view class="other_item">
						<text>联系人</text>
...
<script>
	...
	export default {
		...
		mounted(){
			// let number = uni.getStorageSync("orderID");
			this.load();
		},
		methods: {
			toService(item){
			...
			load(){
				...
				_this.$api.reqPost("api/yl_orders/QueryForDetail",{
					...
					// if(res.status){
					// 	_this.orderList.push(res.data[0]);
					// 	_this.roadList = res.data[1];
